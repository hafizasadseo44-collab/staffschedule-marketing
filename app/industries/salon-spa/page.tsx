    ...
    <section className="relative min-h-screen flex items-center bg-gradient-to-br from-[#0f172a] via-[#1e293b] to-[#0f172a] overflow-hidden pt-24 pb-16">
      <div className="absolute inset-0 pointer-events-none">
        ...
        <motion.div animate={{ scale: [1, 1.22, 1], opacity: [0.15, 0.3, 0.15] }} transition={{ duration: 10, repeat: Infinity, ease: "easeInOut", delay: 1.5 }}
          className="absolute bottom-1/3 right-1/4 w-96 h-96 bg-violet-500/15 rounded-full blur-3xl" />
      </div>
        ...
        <motion.div variants={stagger} initial="hidden" animate="visible" className="text-center lg:text-left">
          ...
          <motion.div variants={fadeUp} className="flex flex-col sm:flex-row gap-3 justify-center lg:justify-start mb-6">
            <Link href="https://app.staffschedule.io/register"
              className="inline-flex items-center justify-center gap-2 px-7 py-3.5 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-indigo-900/40">
          ...
          <motion.div variants={fadeUp} className="flex flex-wrap items-center gap-4 justify-center lg:justify-start text-xs text-indigo-200/60">
            ...
            <span className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-indigo-400" /> Commission-ready</span>
            <span className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-indigo-400" /> Treatment room support</span>
          </motion.div>
        </motion.div>
        ...
        >
          <div className="relative bg-gradient-to-br from-[#1e293b] to-[#0f172a] rounded-2xl border border-indigo-900/40 shadow-2xl shadow-indigo-900/30 overflow-hidden">
            ...
            <div className="px-5 py-4 border-b border-indigo-900/30 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-indigo-200/50 text-[10px]">4 stylists · 13 bookings · 87% chair time</p>
                </div>
              </div>
              <span className="px-2 py-0.5 rounded-full bg-violet-500/15 border border-indigo-500/30 text-indigo-300 text-[10px] font-bold tracking-widest">
                LIVE
              </span>
            </div>

            {/* Stylist column headers */}
            <div className="grid grid-cols-[40px_repeat(4,1fr)] border-b border-indigo-900/30">
              <div></div>
              {STYLISTS.map(s => (
                <div key={s.name} className="px-2 py-2.5 border-l border-emerald-900/20 text-center">
                  ...
                  <p className="text-indigo-200/40 text-[8px] truncate">{s.role}</p>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-[40px_repeat(4,1fr)] relative" style={{ height: 280 }}>
              {/* time labels */}
              {TIME_SLOTS.map((t, i) => (
                <div key={t} className="absolute left-0 w-10 text-right pr-1.5 text-[9px] font-mono text-indigo-300/40"
            ...
            <div className="px-5 py-3 bg-gradient-to-r from-emerald-950/40 to-amber-950/30 border-t border-indigo-900/30 flex items-center justify-between">
              <div>
                <p className="text-indigo-200/50 text-[9px] uppercase tracking-widest">Today&apos;s Chair Utilization</p>
          ...
          <motion.div
            ...
            className="hidden md:flex absolute -right-4 bottom-20 bg-white rounded-2xl shadow-2xl border border-indigo-100 px-3.5 py-2.5 items-center gap-2.5 z-10">
        ...
        <div className="grid lg:grid-cols-5 gap-3">
          {moments.map((m, i) => (
            <motion.button key={i} onClick={() => setMoment(i)}
              ...
              className={`relative text-left p-5 rounded-2xl border-2 transition-all duration-300 ${moment === i ? "border-indigo-300 bg-indigo-50/50 shadow-lg shadow-amber-100 scale-[1.02]" : "border-gray-100 bg-white hover:border-gray-200"}`}
            >
  ...
  return (
    <section className="py-24 bg-gradient-to-br from-[#0f172a] via-[#1e293b] to-[#0f172a] relative overflow-hidden">
      <motion.div animate={{ scale: [1, 1.2, 1], opacity: [0.1, 0.25, 0.1] }} transition={{ duration: 10, repeat: Infinity }}
        className="absolute top-1/3 right-1/3 w-96 h-96 bg-violet-500/15 rounded-full blur-3xl pointer-events-none" />
        ...
        <div className="grid lg:grid-cols-2 gap-14 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}
            ...
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-indigo-300/50 text-[10px]">Senior Stylist · 38 hrs · 22 services</p>
              </div>
              <span className="px-2.5 py-1 rounded-full bg-violet-500/15 border border-indigo-500/30 text-indigo-300 text-[10px] font-bold">
                40% TIER
              </span>
            </div>
              ...
              {[
                ...
                <motion.div key={row.label}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-indigo-300/50 text-[10px]">Revenue: {row.amount}</p>
                  </div>
                  <p className="text-indigo-300 text-sm font-bold tabular-nums">{row.commission}</p>
                </motion.div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <motion.div initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                ...
                className="bg-indigo-500/10 border border-indigo-500/30 rounded-xl p-3 text-center">
                <p className="text-indigo-300/70 text-[9px] uppercase tracking-wider">Commission Total</p>
                <p className="text-indigo-200 text-2xl font-black">$1,730</p>
              </motion.div>
              <motion.div initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                transition={{ delay: 0.9 }}
                className="bg-violet-500/10 border border-violet-500/30 rounded-xl p-3 text-center">
                <p className="text-violet-300/70 text-[9px] uppercase tracking-wider">Hourly Wage</p>
                <p className="text-indigo-200 text-2xl font-black">$760</p>
              </motion.div>
            </div>
            <p className="text-center text-indigo-200/60 text-[10px] mt-3">Both flow to payroll automatically · Friday sync</p>
          </motion.div>
          ...
          <div>
            ...
            <h2 className="text-3xl sm:text-4xl font-black text-white tracking-tight mb-5">
              ...
              <br />
              <span className="bg-gradient-to-r from-indigo-300 to-violet-300 bg-clip-text text-transparent">Calculated Automatically.</span>
            </h2>
...
function FeaturesGrid() {
  const feats = [
    { icon: Calendar, t: "Appointment-Aware Scheduling", d: "Coverage matches your booking density. No overstaffing slow afternoons, no understaffing Saturday morning.", grad: "from-indigo-500 to-violet-600" },
    { icon: DollarSign, t: "Commission & Tip Tracking", d: "Service commissions, retail product commissions, and tip pools — all calculated and synced to payroll.", grad: "from-violet-500 to-purple-600" },
    { icon: Shield, t: "Service Certification Gating", d: "Only certified colorists are scheduled for color services. Med-spa licenses respected. Audit-ready.", grad: "from-violet-500 to-fuchsia-600" },
    { icon: Sparkles, t: "Treatment Room Assignment", d: "Schedule rooms and chairs as resources. No double-bookings. Utilization reports show what&apos;s working.", grad: "from-indigo-500 to-violet-600" },
    { icon: Heart, t: "Client Preferences & VIP Notes", d: "Allergies, formulas, preferred products — visible to whoever takes the appointment.", grad: "from-violet-500 to-fuchsia-500" },
    { icon: Repeat, t: "Appointment-Safe Shift Swaps", d: "Stylist swaps respect existing bookings. A client booked with Maya won&apos;t accidentally be reassigned to a different stylist.", grad: "from-indigo-600 to-violet-700" },
    { icon: ShoppingBag, t: "Retail Upsell Tracking", d: "See which stylists drive product sales. Recognize top performers with data, not guesswork.", grad: "from-violet-600 to-fuchsia-700" },
    { icon: MessageSquare, t: "Front-Desk to Back-Bar Chat", d: "Pre-service notes from reception to stylists. Walk-in capacity shared instantly. No more sticky-note systems.", grad: "from-violet-600 to-purple-700" },
  ];
  return (
    <section className="py-24 bg-violet-50/20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-14">
          ...
          <h2 className="text-3xl sm:text-4xl font-black text-gray-900 tracking-tight mb-4">
            ...
            <span className="bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent">How Salons Actually Run</span>
          </h2>
          <p className="text-gray-500 text-lg max-w-2xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            <div className="text-left">
              ...
              <p className="text-indigo-100/80 text-sm">Owner · 3-location boutique salon</p>
            </div>
          </div>
        </motion.div>
      </div>
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <motion.div initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
          ...
          <h2 className="text-4xl sm:text-5xl font-black text-white mb-5 leading-tight">
            ...
            <br />
            <span className="bg-gradient-to-r from-indigo-300 via-violet-300 to-fuchsia-300 bg-clip-text text-transparent">Beautifully Paid.</span>
          </h2>
